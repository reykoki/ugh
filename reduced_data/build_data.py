...
os.environ['TORCH_HOME'] = '/scratch/alpine/mecr8410/deeplearning/lab4/data/testing/'
...
def encode_answers(answer_list, answers_vocab):
    embed_a = torch.zeros(len(answers_vocab)+1, device=device).long()
...
def get_data_dict(data, vocab, num_annots):
    ...
    for idx, annot in enumerate(data[0:num_annots]):
...
preprocessed_data = get_data_dict(train_data, train_vocab, 10000)
pickle.dump(preprocessed_data, open('pre_proc_train_data1.pickle', 'wb'))
...
preprocessed_data = get_data_dict(val_data, train_vocab, 2000)
pickle.dump(preprocessed_data, open('pre_proc_val_data1.pickle', 'wb'))
